function parseUplink(device, payload) {
    ...
    const waterWeight = data.messages[0][0].measurementValue;

    if (waterWeight < 10000) {
        var evaporationRateEndpoint = device.endpoints.byAddress("1");
        var waterWeightEndpoint = device.endpoints.byAddress("2");
        var waterHeightEndpoint = device.endpoints.byAddress("3");

        var calculatedWaterHeightValue = (waterWeight / 314.1593).toFixed(3);

        // Get previously stored values
        var previousWaterWeight = waterWeightEndpoint.getCurrentState().value || 0;
        var previousWaterHeight = waterHeightEndpoint.getCurrentState().value || 0;

        // Calculate evaporation rate only after the first update
        var evaporationRate = 0;
        if (previousWaterHeight > 0) { 
            evaporationRate = (previousWaterHeight - calculatedWaterHeightValue).toFixed(3);
            if (evaporationRate < 0) evaporationRate = 0; // Avoid negative evaporation rates
        }

        // Update sensor values
        evaporationRateEndpoint.updateGenericSensorStatus(evaporationRate);
        waterWeightEndpoint.updateGenericSensorStatus(waterWeight);
        waterHeightEndpoint.updateGenericSensorStatus(calculatedWaterHeightValue);
    }
}
